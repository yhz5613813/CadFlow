...
<p align="center">
  <img src="docs/assets/readme/cadflow-hero.svg" width="100%" alt="CadFlow overview: external apps and CAD agents send Python programs into the native geometry boundary, receive structured feedback, and produce CAD exchange files, planar DXF profiles, previews, and portable state. The center shows a real two-stage planetary reducer in an exploded display.">
</p>

The central model is generated by the [two-stage planetary reducer example](examples/16_compact_two_stage_planetary_reducer/). Its exploded positions and colors are for illustration; the original assembly state is unchanged. See [figure sources and provenance](docs/assets/readme/README.md).
...
<p align="center">
  <img src="docs/assets/readme/cadflow-geometry-feedback.svg" width="100%" alt="The quick-start plate before and after a cylindrical through cut, alongside the executable Python program and actual native inspection results: volume 31,095.22 cubic millimeters, 7 faces, 15 edges, 10 vertices, and 1 solid.">
</p>

This local run uses millimeters as the design unit. The reported geometry facts illustrate the inspection API; basic validation is not a manufacturability certificate.
...
### Examples built with CadFlow

<p align="center">
  <img src="docs/assets/readme/cadflow-examples.svg" width="100%" alt="Actual CadFlow example renders: a reinforced mounting bracket, an exploded two-stage planetary reducer, and a composed ceramic cup.">
</p>

Explore the source programs: [mounting bracket](examples/cadflow_complex_mounting_bracket.py), [planetary reducer](examples/16_compact_two_stage_planetary_reducer/), and [ceramic cup](examples/cadflow_ceramic_cup.py). These are reproducible geometry examples, not LLM-generation benchmarks; the cup is a multi-solid composition.
...
<p align="center">
  <img src="docs/assets/readme/cadflow-native-boundary.svg" width="100%" alt="CadFlow ownership boundary: Python retains Model, Shape, Graph, and engineering workflows; calls and handles cross the C ABI to a C++ Session that owns native geometry, while facts and buffers return to Python.">
</p>
...
<details>
<summary>End-to-end program, geometry, and artifact flow</summary>

<p align="center">
  <img src="docs/assets/readme/cadflow-overview.svg" width="100%" alt="A mechanism-focused overview of Python API calls and typed operation graphs converging on CadFlow's native geometry engine, producing engineering artifacts and returning structured feedback to the external caller.">
</p>
...
<p align="center">
  <img src="docs/assets/readme/cadflow-agent-dsl.svg" width="100%" alt="The optional experimental Agent DSL compiles incremental commands into revisioned model state, returns bounded JSON facts, and publishes revision-safe Scene previews. Rollback restores checkpoint content into a new revision rather than rewinding revision numbers.">
</p>

The DSL wrapper is not installed with the core SDK. Its checkpoints restore model content while revision numbers continue to advance; full Model JSON remains an explicit export. See the [Agent DSL documentation](agent_dsl/README.md).
