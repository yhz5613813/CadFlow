...
本文档用于汇总并冻结 `CadFlow 2.0` 的重构方向。核心目标不是继续围绕 `OCP` 做包装，而是把系统重构为一个以 `OCP` 为底层内核、以参数/表达式/操作图/拓扑历史为事实来源的建模系统。
...
- `src/cadflow/geometry.py`
- `src/cadflow/frame.py`
- `src/cadflow/sketch.py`

### OCP 内核适配

- `src/cadflow/kernel/ocp_shapes.py`
- `src/cadflow/kernel/ocp_query.py`
- `src/cadflow/kernel/ocp_builders.py`

### 基础操作

- `src/cadflow/ops/primitives.py`
- `src/cadflow/ops/curves.py`
- `src/cadflow/ops/features.py`
- `src/cadflow/ops/booleans.py`
- `src/cadflow/ops/transforms.py`

### 历史与拓扑

- `src/cadflow/history/op_graph.py`
- `src/cadflow/history/topology.py`
- `src/cadflow/history/tracking.py`

### 装配与约束

- `src/cadflow/assembly/model.py`
- `src/cadflow/assembly/anchors.py`
- `src/cadflow/assembly/constraints.py`
- `src/cadflow/assembly/solver.py`

### IO 与 replay

- `src/cadflow/io/model_json.py`
- `src/cadflow/io/replay.py`
- `src/cadflow/io/exporters.py`
...
`CadFlow 2.0` 的本质不是简单地“把 `cadquery` 换成 `OCP`”，而是把系统从一个以最终几何为中心的 CQ 包装层，重构成一个以：
...
同时，2.0 不能丢失 `CadFlow` 当前最重要的设计资产：
