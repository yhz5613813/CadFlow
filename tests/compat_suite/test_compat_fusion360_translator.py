...
class TestFusion360Translator(unittest.TestCase):
    ...
    def test_payload_uses_canonical_periodic_curve_frames(self):
        ...
        script = (
            Fusion360Translator()
            .translate_model_json(cad.export_model_json(session))
            .content
        )
    ...
    def test_fallback_scripts_are_deterministic_and_result_scoped(self):
        with GraphSession() as session:
            ...
            cad.revolve_rsolid(profile, axis=(0.0, 1.0, 0.0))
            cad.make_box_rsolid(width=4.0, height=5.0, depth=6.0)
        model_json = cad.export_model_json(session)
        payload = cad.import_model_json(model_json)
        graph = payload["graph"]
        result_id = next(
            node.node_id
    ...
    def test_explicit_result_only_executes_its_dependency_closure(self):
        with GraphSession() as session:
            profile = cad.make_rectangle_rface(width=2.0, height=1.0)
            cad.extrude_rsolid(profile, (0.0, 0.0, 1.0), 3.0)
    ...
    def test_unsupported_result_operation_is_rejected_before_host_execution(self):
        ...
        with self.assertRaises(TranslationRequestError):
            Fusion360Translator().translate_model_json(cad.export_model_json(session))
