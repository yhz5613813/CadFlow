...
class TestGraphModelApi(unittest.TestCase):
    def test_model_export_dir_writes_one_self_contained_scene_package(self):
        with TemporaryDirectory() as directory:
            @cad.model(graph_id="auto_export", export_dir=directory)
            def build_model():
                body = cad.make_box_rsolid(width=1.0, height=2.0, depth=3.0)
    ...
    def test_model_export_dir_writes_assembly_artifacts_with_model_provenance(self):
        with TemporaryDirectory() as directory:
            @cad.model(graph_id="assembly_export", export_dir=directory)
            def build_model():
                body = cad.make_box_rsolid(width=1.0, height=2.0, depth=3.0)
                part = cad.make_part_rpart(part_id="body", body=body)
    ...
    def test_model_source_embedding_is_explicit(self):
        @cad.model(graph_id="embedded_source")
        def build_model():
            body = cad.make_box_rsolid(width=1.0, height=2.0, depth=3.0)
            cad.capture_result(value=body)
            return body
    ...
    def test_model_owns_one_session_and_replays_explicit_result(self):
        @cad.model(graph_id="decorator_model")
        def build_model():
            cad.make_box_rsolid(width=0.5, height=0.5, depth=0.5)
            return cad.make_box_rsolid(width=1.0, height=2.0, depth=3.0)

        result = build_model()

        self.assertIsInstance(result, cad.ModelResult)
    ...
    def test_requires_session_reuses_active_session(self):
        ...
        def build_box():
            return cad.make_box_rsolid(width=1.0, height=1.0, depth=1.0)

        with self.assertRaisesRegex(RuntimeError, "requires an active GraphSession"):
            build_box()

        with cad.GraphSession(graph_id="shared") as session:
            box = build_box()
    ...
    def test_capture_result_can_exclude_unrelated_leaf(self):
        @cad.model
        def build_model():
            debug = cad.make_box_rsolid(width=0.25, height=0.25, depth=0.25)
            final = cad.make_box_rsolid(width=2.0, height=2.0, depth=2.0)
            cad.capture_result(value=final)
            return debug, final
    ...
    def test_cross_session_shape_input_is_rejected_at_operation_boundary(self):
        with cad.GraphSession(graph_id="source"):
            foreign = cad.make_box_rsolid(width=1.0, height=1.0, depth=1.0)

        with cad.GraphSession(graph_id="target"):
            with self.assertRaisesRegex(ValueError, "source.*target"):
                cad.translate_shape(shape=foreign, vector=(1.0, 0.0, 0.0))

    def test_cross_session_child_assembly_is_rejected(self):
        with cad.GraphSession(graph_id="child_graph"):
            child = cad.make_assembly_rassembly(assembly_id="child")

        with cad.GraphSession(graph_id="parent_graph"):
            parent = cad.make_assembly_rassembly(assembly_id="parent")
            placement = cad.identity_placement_rplacement()
    ...
    def test_unrecorded_child_assembly_is_rejected(self):
        child = cad.make_assembly_rassembly(assembly_id="unrecorded_child")
    ...
    def test_capture_result_walks_dataclass_values_and_is_atomic_on_failure(self):
        ...
        @cad.model(graph_id="dataclass_result")
        def build_model():
            body = cad.make_box_rsolid(width=1.0, height=2.0, depth=3.0)
            cad.capture_result(value=ResultValue(body=body))
        ...
        with cad.GraphSession(graph_id="atomic_capture") as session:
            body = cad.make_box_rsolid(width=1.0, height=1.0, depth=1.0)
            with cad.GraphSession(graph_id="foreign_capture"):
                foreign = cad.make_box_rsolid(
        ...
        with cad.GraphSession(graph_id="topology_capture") as session:
            body = cad.make_box_rsolid(width=1.0, height=1.0, depth=1.0)
    ...
    def test_model_and_requires_session_reject_async_functions(self):
        ...
        with self.assertRaisesRegex(TypeError, "@model does not support async"):
            cad.model(async_model)
        with self.assertRaisesRegex(
            TypeError, "@requires_session does not support async"
        ):
            cad.requires_session(async_builder)
