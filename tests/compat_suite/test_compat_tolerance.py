...
class TestDimensionToleranceDeclaration(unittest.TestCase):
    def test_symmetric_and_asymmetric_tolerances(self):
        symmetric = cad.var("width", 10.0, tolerance=0.2)
        asymmetric = cad.var("shaft", 8.0, tolerance=(-0.05, 0.0))
    ...
    def test_dimension_tolerance_roundtrip(self):
        tolerance = cad.DimensionTolerance(-0.1, 0.3)
        self.assertEqual(
            cad.DimensionTolerance.from_dict(tolerance.to_dict()), tolerance
        )
        self.assertAlmostEqual(tolerance.width, 0.4)

    def test_invalid_variable_and_tolerance_values_are_rejected(self):
        invalid_calls = [
            lambda: cad.var("x", True),
            lambda: cad.var("x", math.inf),
            lambda: cad.var("x", 1.0, tolerance=True),
            lambda: cad.var("x", 1.0, tolerance=-0.1),
            lambda: cad.var("x", 1.0, tolerance=(0.1, 0.2)),
            lambda: cad.var("x", 1.0, tolerance=(-0.2, -0.1)),
            lambda: cad.var("x", 1.0, tolerance=(-0.1, 0.1, 0.2)),
            lambda: cad.DimensionTolerance(float("nan"), 0.1),
        ]
    ...
    def test_expression_graph_preserves_variable_tolerances_and_forward_refs(self):
        ...
        self.assertIsInstance(rebuilt_width, cad.Var)
    ...
    def test_var_positional_expr_id_remains_compatible(self):
        variable = cad.Var("width", 10.0, "plate width", "var_width")
    ...
    def test_expression_registration_is_atomic_for_conflicting_ids(self):
        left = cad.Var(
            ...
            tolerance=cad.DimensionTolerance.symmetric(0.1),
        )
        right = cad.Var(
            "right",
            2.0,
        ...
        expression = cad.Expr("add", (left, right))
        graph = cad.ExpressionGraph()
    ...
    def test_expression_registration_rejects_cycles_without_mutation(self):
        expression = cad.Expr("neg", (cad.const(1.0),), expr_id="expr_cycle")
    ...
    def test_expression_arguments_are_normalized_to_an_immutable_tuple(self):
        arguments = [cad.const(1.0)]
        expression = cad.Expr("neg", arguments)
    ...
    def test_signed_zero_nodes_with_the_same_id_are_distinct(self):
        positive = cad.Const(0.0, expr_id="const_zero")
        negative = cad.Const(-0.0, expr_id="const_zero")
        graph = cad.ExpressionGraph()
...
class TestWorstCaseTolerancePropagation(unittest.TestCase):
    def test_addition_and_subtraction_preserve_asymmetric_deviations(self):
        a = cad.var("a", 10.0, tolerance=(-0.1, 0.2))
        b = cad.var("b", 5.0, tolerance=(-0.3, 0.4))

        result = cad.analyze_tolerance(a - b)
    ...
    def test_linear_scaling_and_repeated_variable_dependency_are_exact(self):
        x = cad.var("x", 4.0, tolerance=(-0.2, 0.3))

        scaled = cad.analyze_tolerance(-2.0 * x + 1.0)
        cancelled = cad.analyze_tolerance(x - x)
    ...
    def test_multiplication_handles_all_interval_corner_signs(self):
        a = cad.var("a", -1.0, tolerance=(-1.0, 0.5))
        b = cad.var("b", 3.0, tolerance=(-1.0, 1.0))
    ...
    def test_division_rejects_denominator_interval_containing_zero(self):
        numerator = cad.var("numerator", 2.0, tolerance=0.1)
        denominator = cad.var("denominator", 1.0, tolerance=1.0)

        with self.assertRaisesRegex(
            cad.ToleranceAnalysisError, "denominator.*contains zero"
        ):
            cad.analyze_tolerance(numerator / denominator)

    def test_integer_and_fractional_power_intervals(self):
        signed = cad.var("signed", 0.0, tolerance=2.0)
        positive = cad.var("positive", 4.0, tolerance=(-3.0, 5.0))

        squared = cad.analyze_tolerance(signed**2)
        rooted = cad.analyze_tolerance(cad.sqrt(positive))
    ...
    def test_power_domain_errors_are_explicit(self):
        negative = cad.var("negative", -2.0, tolerance=0.1)
        crosses_zero = cad.var("crosses_zero", 0.0, tolerance=0.1)

        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "negative base"):
            cad.analyze_tolerance(negative**0.5)
        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "zero"):
            cad.analyze_tolerance(crosses_zero ** -1)

    def test_invalid_constant_subexpressions_use_analysis_errors(self):
        expression = cad.sqrt(-1.0)

        with self.assertRaisesRegex(
            cad.ToleranceAnalysisError, "declared tolerance interval"
        ):
            cad.analyze_tolerance(expression)

    def test_trigonometric_extrema_and_discontinuities(self):
        angle = cad.var("angle", 0.0, tolerance=(-math.pi, math.pi))
        tangent_angle = cad.var(
            "tangent_angle", math.pi / 2.0, tolerance=0.1
        )

        sine = cad.analyze_tolerance(cad.sin(angle))
        cosine = cad.analyze_tolerance(cad.cos(angle))

        self.assertEqual((sine.lower_bound, sine.upper_bound), (-1.0, 1.0))
        self.assertEqual((cosine.lower_bound, cosine.upper_bound), (-1.0, 1.0))
        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "discontinuity"):
            cad.analyze_tolerance(cad.tan(tangent_angle))

    def test_inverse_function_domains_are_checked_over_full_interval(self):
        below_sqrt = cad.var("below_sqrt", 0.1, tolerance=0.2)
        outside_unit = cad.var("outside_unit", 0.9, tolerance=0.2)

        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "below zero"):
            cad.analyze_tolerance(cad.sqrt(below_sqrt))
        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "outside"):
            cad.analyze_tolerance(cad.asin(outside_unit))
        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "outside"):
            cad.analyze_tolerance(cad.acos(outside_unit))

    def test_atan_and_atan2_propagation(self):
        y = cad.var("y", 2.0, tolerance=0.1)
        x = cad.var("x", 3.0, tolerance=0.2)
        origin_y = cad.var("origin_y", 0.0, tolerance=0.1)
        origin_x = cad.var("origin_x", 0.0, tolerance=0.1)

        atan_result = cad.analyze_tolerance(cad.atan(y))
        atan2_result = cad.analyze_tolerance(cad.atan2(y, x))
        ...
        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "undefined origin"):
            cad.analyze_tolerance(cad.atan2(origin_y, origin_x))

    def test_atan2_negative_x_branch_cut_is_not_underestimated(self):
        y = cad.var("y", 0.0, tolerance=0.1)
        x = cad.var("x", -2.0, tolerance=0.1)
        expression = cad.atan2(y, x)

        result = cad.analyze_tolerance(expression)

        self.assertEqual(result.lower_bound, -math.pi)
        self.assertEqual(result.upper_bound, math.pi)
        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "branch cut"):
            cad.analyze_tolerance(expression, method="rss")

    def test_atan2_signed_zero_branch_cut_is_rejected_for_rss(self):
        y = cad.var("y", -0.0, tolerance=(0.0, 0.1))
        x = cad.var("x", -2.0, tolerance=0.0)
        expression = cad.atan2(y, x)

        worst_case = cad.analyze_tolerance(expression)
    ...
    def test_abs_crossing_zero_and_negation(self):
        value = cad.var("value", 0.25, tolerance=0.5)

        absolute = cad.analyze_tolerance(abs(value))
        negated = cad.analyze_tolerance(-value)
    ...
    def test_nonlinear_contributions_are_reported_in_target_units(self):
        value = cad.var("value", 2.0, tolerance=0.1)

        result = cad.analyze_tolerance(value**2)
    ...
    def test_large_nominal_value_does_not_erase_small_declared_deviations(self):
        value = cad.var("large", 1e16, tolerance=0.1)

        result = cad.analyze_tolerance(value)
    ...
    def test_underflowed_nonlinear_range_remains_conservative(self):
        value = cad.var("tiny", 0.0, tolerance=1e-200)

        result = cad.analyze_tolerance(value**2)
        check = cad.check_tolerance(value**2, 0.0)
    ...
    def test_large_atan_input_keeps_nonzero_worst_case_range(self):
        value = cad.var("large", 1e160, tolerance=1e100)

        result = cad.analyze_tolerance(cad.atan(value))

        self.assertLess(result.lower_deviation, 0.0)
        self.assertGreater(result.upper_deviation, 0.0)

    def test_zero_tolerance_large_periodic_input_remains_a_point(self):
        angle = cad.var("angle", 1e16, tolerance=0.0)

        tangent = cad.analyze_tolerance(cad.tan(angle))
        sine = cad.analyze_tolerance(cad.sin(angle))
    ...
    def test_exact_zero_absolute_value_is_not_spuriously_widened(self):
        value = cad.var("zero", 0.0, tolerance=0.0)

        result = cad.analyze_tolerance(abs(value))
...
class TestRssTolerancePropagation(unittest.TestCase):
    def test_independent_sources_are_combined_by_root_sum_square(self):
        a = cad.var("a", 10.0, tolerance=0.3)
        b = cad.var("b", 4.0, tolerance=0.4)

        result = cad.analyze_tolerance(a + b, method="rss")

        self.assertAlmostEqual(result.lower_deviation, -0.5)
        self.assertAlmostEqual(result.upper_deviation, 0.5)

    def test_repeated_source_is_not_treated_as_independent(self):
        x = cad.var("x", 2.0, tolerance=0.2)

        cancelled = cad.analyze_tolerance(x - x, method="rss")
        doubled = cad.analyze_tolerance(x + x, method="rss")
    ...
    def test_nonlinear_sensitivity_is_analytic(self):
        ...
        squared = cad.analyze_tolerance(x**2, method="rss")
        sine = cad.analyze_tolerance(cad.sin(x), method="rss")
    ...
    def test_rss_rejects_non_differentiable_and_invalid_intervals(self):
        at_zero = cad.var("at_zero", 0.0, tolerance=0.1)
        denominator = cad.var("denominator", 1.0, tolerance=1.0)
        numerator = cad.var("numerator", 2.0, tolerance=0.1)

        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "abs"):
            cad.analyze_tolerance(abs(at_zero), method="rss")
        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "denominator"):
            cad.analyze_tolerance(numerator / denominator, method="rss")

    def test_all_unary_and_binary_derivative_paths_are_supported(self):
        a = cad.var("a", 0.5, tolerance=0.01)
        b = cad.var("b", 2.0, tolerance=0.02)
        expressions = [
            a + b,
            a - b,
            a * b,
        ...
        for expression in expressions:
            with self.subTest(op=getattr(expression, "op", None)):
                result = cad.analyze_tolerance(expression, method="rss")
    ...
    def test_constant_boundary_functions_have_zero_rss_tolerance(self):
        expressions = [cad.sqrt(0.0), cad.acos(1.0), cad.asin(-1.0)]
    ...
    def test_rss_uses_overflow_safe_root_sum_square(self):
        a = cad.var("a", 0.0, tolerance=1e200)
        b = cad.var("b", 0.0, tolerance=1e200)
    ...
    def test_rss_atan2_derivative_is_stable_for_large_coordinates(self):
        y = cad.var("y", 1e200, tolerance=1e190)
        x = cad.var("x", 1e200, tolerance=1e190)

        result = cad.analyze_tolerance(cad.atan2(y, x), method="rss")
    ...
    def test_rss_atan_derivative_does_not_underflow_before_contribution(self):
        ...
        result = cad.analyze_tolerance(cad.atan(value), method="rss")
    ...
    def test_ill_conditioned_affine_coefficients_preserve_source_effect(self):
        value = cad.var("value", 1.0, tolerance=0.1)
        expression = (1e16 * value + value) - 1e16 * value

        worst_case = cad.analyze_tolerance(expression)
        rss = cad.analyze_tolerance(expression, method="rss")
...
class TestToleranceRequirementsAndPersistence(unittest.TestCase):
    def test_check_tolerance_returns_margins_without_raising(self):
        a = cad.var("a", 10.0, tolerance=0.2)
        b = cad.var("b", 5.0, tolerance=0.1)

        passing = cad.check_tolerance(a - b, 0.3, name="clearance")
        failing = cad.check_tolerance(a - b, 0.2, name="clearance")
    ...
    def test_requirement_comparison_does_not_scale_epsilon_by_nominal(self):
        ...
        check = cad.check_tolerance(value, 0.0)
    ...
    def test_analysis_rejects_conflicting_expression_ids_before_caching(self):
        ...
        right = cad.Var(
            ...
            2.0,
            expr_id="var_duplicate",
            tolerance=cad.DimensionTolerance.symmetric(0.2),
        )

        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "multiple"):
            cad.analyze_tolerance(cad.Expr("add", (left, right)))

    def test_every_variable_requires_an_explicit_source_tolerance(self):
        declared = cad.var("declared", 2.0, tolerance=0.1)
        missing = cad.var("missing", 1.0)

        with self.assertRaisesRegex(cad.ToleranceAnalysisError, "missing"):
            cad.analyze_tolerance(declared + missing)

    def test_tolerance_graph_validates_all_requirements(self):
        a = cad.var("a", 10.0, tolerance=0.2)
        b = cad.var("b", 5.0, tolerance=0.1)
        expression_graph = cad.ExpressionGraph()
        tolerance_graph = cad.ToleranceGraph(expression_graph)
        ...
        with self.assertRaises(cad.ToleranceValidationError) as context:
    ...
    def test_tolerance_graph_roundtrip_and_dangling_refs(self):
        ...
        rebuilt_expression_graph = cad.ExpressionGraph.from_dict(expression_payload)
        rebuilt = cad.ToleranceGraph.from_dict(
        ...
        with self.assertRaisesRegex(ValueError, "Unknown tolerance target"):
            cad.ToleranceGraph.from_dict(broken, rebuilt_expression_graph)
    ...
    def test_tolerance_graph_rejects_malformed_requirement_types(self):
        ...
        for field, invalid in (
            ...
            with self.subTest(field=field), self.assertRaises((TypeError, ValueError)):
                cad.ToleranceGraph.from_dict(payload, expression_graph)

    def test_duplicate_requirement_ids_are_rejected(self):
        width = cad.var("width", 10.0, tolerance=0.1)
        graph = cad.ToleranceGraph(cad.ExpressionGraph())
    ...
    def test_session_and_model_json_include_tolerance_graph(self):
        ...
        session_payload = cad.import_session_json(cad.export_session_json(session))
        model_payload = cad.import_model_json(cad.export_model_json(session))
    ...
    def test_legacy_session_and_model_payloads_default_to_empty_tolerance_graph(self):
        with cad.GraphSession() as session:
            pass
        raw_session = json.loads(cad.export_session_json(session))
        raw_model = json.loads(cad.export_model_json(session))
        raw_session.pop("tolerance_graph")
        raw_model.pop("tolerance_graph")

        imported_session = cad.import_session_json(json.dumps(raw_session))
        imported_model = cad.import_model_json(json.dumps(raw_model))
    ...
    def test_failed_requirement_blocks_session_and_model_export(self):
        width = cad.var("width", 10.0, tolerance=0.2)
        with cad.GraphSession() as session:
            session.require_tolerance(width, 0.1, name="width")

        with self.assertRaises(cad.ToleranceValidationError):
            cad.export_session_json(session)
    ...
    def test_failed_requirement_in_hand_edited_model_blocks_replay(self):
        width = cad.var("width", 10.0, tolerance=0.2)
        with cad.GraphSession() as session:
            cad.make_box_rsolid(width, 1.0, 1.0)
    ...
    def test_analysis_and_report_payloads_are_json_serializable(self):
        width = cad.var("width", 10.0, tolerance=0.1)
        check = cad.check_tolerance(width * 2.0, 0.2, name="overall")
    ...
    def test_unknown_method_is_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "Unsupported"):
            cad.analyze_tolerance(width, method="monte_carlo")
