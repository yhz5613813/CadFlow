...
from cadflow import extrude_rsolid, make_box_rsolid, make_line_redge, make_rectangle_rface
from cadflow.core import Edge, Face, Solid, Wire


def test_public_shapes_do_not_expose_cadquery_accessors() -> None:
    ...
    for shape in (box, face, edge, vertex, wire):
        ...
        for forbidden in ("cq_solid", "cq_face", "cq_edge", "cq_vertex", "cq_wire"):
            assert not hasattr(shape, forbidden), (
                f"{type(shape).__name__} exposes {forbidden}"
            )


def test_ocp_native_basic_properties() -> None:
...
def test_ocp_native_face_wire_topology_and_extrude() -> None:
...
def test_engine_source_has_no_cadquery_dependency() -> None:
    engine_root = Path(__file__).resolve().parents[1] / "python/cadflow/_engine"
    source_files = [
        path for path in engine_root.rglob("*.py") if "tools" not in path.parts
    ]
    forbidden_tokens = (
        "import cadquery",
        "from cadquery",
