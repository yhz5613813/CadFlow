...
SCHEMA_DIR = ROOT / "python/cadflow/_engine/exchange/scene/contracts/schemas"
PYTHON_OUTPUT = ROOT / "python/cadflow/_engine/exchange/scene/generated"
...
def main() -> int:
    ...
    if args.language in {"all", "python"}:
        try:
            ...
            print(
                "datamodel-code-generator is not installed; keeping the checked-in Python types",
                file=sys.stderr,
            )
            if args.check:
                return 0
            raise RuntimeError(
                "install datamodel-code-generator to regenerate Python types"
            )
